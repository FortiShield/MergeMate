...
...
    steps:
      - name: PR Agent action step
        id: mergemate
        uses: Khulnasoft/mergemate@v2.0
...
```
...
4) You may configure MergeMate by adding environment variables under the env section corresponding to any configurable property in the [configuration](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/settings/configuration.toml) file. Some examples:
```yaml
      env:
...
```
See detailed usage instructions in the [USAGE GUIDE](https://khulnasoft.github.io/mergemate/usage-guide/automations_and_usage/#github-action)
...
```
   ...
   - Set deployment_type to 'app' in [configuration.toml](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/settings/configuration.toml)
...
> **Note:** When running MergeMate from GitHub App, the default configuration file (configuration.toml) will be loaded.
> However, you can override the default tool parameters by uploading a local configuration file `.mergemate.toml`
