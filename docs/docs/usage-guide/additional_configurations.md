...
1) [Use a model](https://khulnasoft.github.io/Docs-MergeMate/usage-guide/#changing-a-model) with larger context, like GPT-32K, or claude-100K. This solution will be applicable for all the tools.
2) For the `/improve` tool, there is an ['extended' mode](https://khulnasoft.github.io/Docs-MergeMate/tools/#improve) (`/improve --extended`),
...
See [here](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/algo/__init__.py) for the list of available models.
To use a different model than the default (GPT-4), you need to edit [configuration file](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/settings/configuration.toml#L2).
...
Also, review the [AiHandler](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/algo/ai_handler.py) file for instructions on how to set keys for other models.
...
If the PR is too large (see [PR Compression strategy](https://github.com/Khulnasoft/mergemate/blob/main/PR_COMPRESSION.md)), MergeMate automatically sets this number to 0, using the original git patch.
...
For example, if you want to edit the prompts of the [describe](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/settings/pr_description_prompts.toml) tool, you can add the following to your `.mergemate.toml` file:
```
...
"""
```
Note that the new prompt will need to generate an output compatible with the relevant [post-process function](https://github.com/Khulnasoft/mergemate/blob/main/mergemate/tools/pr_description.py#L137).
