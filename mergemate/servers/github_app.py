...
from mergemate.servers.utils import DefaultDictWithTimeout, verify_signature
...
def handle_closed_pr(body, event, action, log_context):
    ...
    log_context["api_url"] = api_url
    get_logger().info("MergeMate statistics for closed PR", analytics=True, pr_statistics=pr_statistics, **log_context)
...
async def _perform_auto_commands_github(commands_conf: str, agent: MergeMate, body: dict, api_url: str, log_context: dict):
